...
func NewMemorySpaceExplorer(memorySpace MemorySpace, target Target) MemorySpaceExplorer {
...
func (this MemorySpaceExplorer) FindLowestCostToReachTarget() int {
	for len(this.toVisitStack) > 0 {
		var toVisit = this.toVisitStack.PopFirstElement()

		if toVisit.coordinate == this.target {
			return toVisit.cost
		}
